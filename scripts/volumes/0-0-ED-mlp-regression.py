...
for i in range(len(data_ED_train)):
    data_ED_train[i] = data_ED_train[i][shuffle_index]
...
random.shuffle(shuffle_index)
for i in range(len(data_ED_train_val)):
   data_ED_train_val[i] = data_ED_train_val[i][shuffle_index]
...
model_ED_value_regression = Graph()

for i in range(nb_slices_z):
    model_ED_value_regression.add_input(name='input_mask_{}'.format(i), input_shape=(1, img_size, img_size))

for i in range(nb_slices_z):
    model_ED_value_regression.add_node(Flatten(), 
                                       name='apply_mask_{}'.format(i), 
                                       input='input_mask_{}'.format(i))

model_ED_value_regression.add_node(Dense(1, activation='linear'), 
                                   name='pre-output', 
                                   inputs=['apply_mask_{}'.format(i) for i in range(nb_slices_z)],
                                   merge_mode='concat',
                                   concat_axis=-1)
model_ED_value_regression.add_output(name='output', input='pre-output')

model_ED_value_regression.compile('adam', {'output': RMSE})
...
checkpointer = ModelCheckpoint(filepath='../../model_weights/0-model_ED_mlp_regression.hdf5',
                               verbose=1, save_best_only=True)
earlystopping = EarlyStopping(monitor='val_loss', patience=10, verbose=1)

train_obj = {}
for i in range(nb_slices_z):
    train_obj['input_mask_{}'.format(i)] = data_ED_train[2*i+1]
train_obj['output'] = labels_value_ED_train

train_val_obj = {}
for i in range(nb_slices_z):
    train_val_obj['input_mask_{}'.format(i)] = data_ED_train_val[2*i+1]
train_val_obj['output'] = labels_value_ED_train_val

model_ED_value_regression.fit(train_obj,
