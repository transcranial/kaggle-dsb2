...
model_ES_value_regression = Graph()

for i in range(nb_slices_z):
    model_ES_value_regression.add_input(name='input_mask_{}'.format(i), input_shape=(1, img_size, img_size))

for i in range(nb_slices_z):
    model_ES_value_regression.add_node(Flatten(), 
                                       name='apply_mask_{}'.format(i), 
                                       input='input_mask_{}'.format(i))

model_ES_value_regression.add_node(Dense(1, activation='linear'), 
                                   name='pre-output', 
                                   inputs=['apply_mask_{}'.format(i) for i in range(nb_slices_z)],
                                   merge_mode='concat',
                                   concat_axis=-1)
model_ES_value_regression.add_output(name='output', input='pre-output')

model_ES_value_regression.compile('adam', {'output': RMSE})
...
train_obj['output'] = labels_value_ES_train

train_val_obj = {}
for i in range(nb_slices_z):
    train_val_obj['input_img_{}'.format(i)] = data_ES_train_val[2*i]
    train_val_obj['input_mask_{}'.format(i)] = data_ES_train_val[2*i+1]
train_val_obj['output'] = labels_value_ES_train_val

model_ES_value_regression.fit(train_obj,
