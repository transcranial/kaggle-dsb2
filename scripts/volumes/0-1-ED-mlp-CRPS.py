...
model_ED_func_regression = Graph()

for i in range(nb_slices_z):
    model_ED_func_regression.add_input(name='input_mask_{}'.format(i), input_shape=(1, img_size, img_size))

for i in range(nb_slices_z):
    model_ED_func_regression.add_node(Flatten(), 
                                       name='apply_mask_{}'.format(i), 
                                       input='input_mask_{}'.format(i))

model_ED_func_regression.add_node(Dense(600, activation='softmax'), 
                                   name='pre-output', 
                                   inputs=['apply_mask_{}'.format(i) for i in range(nb_slices_z)],
                                   merge_mode='concat',
                                   concat_axis=-1)
model_ED_func_regression.add_output(name='output', input='pre-output')

model_ED_func_regression.compile('adam', {'output': CRPS})
...
model_ED_func_regression.fit(train_obj,
